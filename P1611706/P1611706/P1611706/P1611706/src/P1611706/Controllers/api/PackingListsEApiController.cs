...
namespace P1611706.Controllers.api
{
    ...
    public class PackingListsEApiController : BaseController
    {
		private P1611706DB _db = null;

		public PackingListsEApiController(P1611706DB db)
		{
			_db = db;
		}
		...
		public JsonResult Get(int draw, int start)
		{
            ...
            rows = tmp.OrderBy(e => e.CompanyCode).OrderBy(e => e.PackingTime).OrderBy(e => e.ProductNo).OrderBy(e => e.Lot).ToList();
			...
			foreach (PackingPallets packingPallets in rows)
			{
				...
				if (newPackingPallets.CompanyCode.Equals(packingPallets.CompanyCode) &&
					...
					newPackingPallets.ProductNo.Equals(packingPallets.ProductNo) &&
